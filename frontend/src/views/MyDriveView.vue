...
<template>
	<DriveShell current-section="drive" @new-folder="createNewFolder" @upload-files="openFilePicker" @upload-folder="openFolderPicker">
		<div id="MyDriveView" class="relative min-h-[calc(100vh-84px)] scroll-mt-20 rounded-3xl border border-[rgba(245,225,205,0.09)] bg-[#1e1712] p-6 text-[#fcf7f0] shadow-xl sm:p-8" @click="clearSelection" @dragenter.prevent="handleDragEnter" @dragover.prevent="handleDragEnter" @dragleave.prevent="handleDragLeave" @drop.prevent="handleDrop">
			...
			<div v-if="isDragActive" class="pointer-events-none absolute inset-4 z-20 grid place-items-center rounded-3xl border-2 border-dashed border-[#f97316] bg-[#261d17]/95 text-center backdrop-blur-md">
				<div>
					<p class="text-lg font-bold text-[#f97316]">Lepas file di sini untuk upload</p>
					<p class="mt-2 text-sm text-[#b8a693]">File dan folder akan diunggah ke lokasi Drive saat ini.</p>
				</div>
			</div>

			<div class="mb-4 flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
				<nav aria-label="Breadcrumb" class="m-0 flex flex-wrap items-center gap-1 text-2xl font-bold text-[#fcf7f0]">
					<template v-for="(crumb, index) in breadcrumbs" :key="crumb.path">
						<button type="button" class="max-w-[220px] truncate text-left transition hover:text-[#f97316]" @click="fileTreeStore.navigate(crumb.path)">{{ crumb.label === 'Root' ? 'Drive Saya' : crumb.label }}</button>
						<IconChevronRight v-if="index < breadcrumbs.length - 1" :size="18" :stroke="2" class="mx-1 text-[#b8a693]" />
					</template>
				</nav>
				...
				<div class="custom-scrollbar overflow-x-auto rounded-2xl border border-[rgba(245,225,205,0.09)] bg-[#1e1712]">
					<div class="min-w-[760px]">
						<div class="custom-scrollbar max-h-[min(70vh,780px)] overflow-y-auto overflow-x-hidden" @scroll="handleListScroll">
							...
							<div v-if="!sortedFiles.length && !isLoading" class="p-6 text-center text-sm text-[#b8a693]">{{ t('drive.noFiles') }}</div>
